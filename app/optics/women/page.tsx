...
export default function WomensOpticsPage() {
  const { products } = useProducts()
  const base = products.filter((p) => p.category === "optics" && p.gender === "women")
  ...
  return (
    <div className="mx-auto max-w-7xl gap-6 px-4 py-8 md:grid md:grid-cols-[280px_1fr]">
      <FiltersPanel
        products={base}
        config={{ search: true, material: true, color: true, shape: true, price: true, sort: true }}
        accessors={logic.accessors}
        facets={logic.facets}
        className="mb-6 md:mb-0"
      />
      <div>
        <h1 className="text-xl font-semibold">Women’s Optics</h1>
        <div className="mt-4">
